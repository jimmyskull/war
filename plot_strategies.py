...
def make_plot():
    ...
    for strat in config.STRATEGIES:
        strat.load_cache()
        best_scores.append(strat.cache['best']['agg']['avg'])
        for _, result in strat.database.iterate():
    ...
    for stid, strat in enumerate(config.STRATEGIES):
        history = list()
        for (oid, result) in strat.database.iterate():
            if result['type'] != 'result':
                continue
            result = result['data']
            if result['status'] != 'OK':
...
def save_plot():
    fig, legend = make_plot()
    fig.savefig('strategies.png', quality=100, dpi=100,
                bbox_extra_artists=(legend,),
                bbox_inches='tight')


def war_monitor(interval=1):
    """
    Function to be called within a Jupyter notebook.
    ...
    Example
    --------
    >>> from plot_strategies import war_monitor
    >>> war_monitor()
    """
